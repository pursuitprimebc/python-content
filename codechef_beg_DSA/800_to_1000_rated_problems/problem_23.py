''' QUESTION - Chef and Contest
Chef and Chefina are competing against each other in a programming contest. They were both able to solve all the problems in the contest, so the winner between them must be 
decided by time penalty. Chef solved all the problems in X minutes and made P wrong submissions, while Chefina solved all the problems in Y minutes and madeQ wrong submissions.
...
for i in range(t):
    ...
    penalty_time_for_chef = x+(p*10)
    penalty_time_for_chefina = y+(q*10)
    if penalty_time_for_chefina<penalty_time_for_chef:
        print('chefina')
    elif penalty_time_for_chef<penalty_time_for_chefina:
        print('chef')
    else:
        print('draw')
